...
(function () {
  ...
  if (window.__vimeoInjected) {
    // Si ya estaba inyectado, hacer rescan inmediato
  ...
  function scanDOM() {
    ...
    // Scripts inline con JSON de configuración
    document.querySelectorAll('script:not([src])').forEach(s => {
      const t = s.textContent;
      if (!t || t.length < 100) return;
      // Patrones de config conocidos
      const patterns = [
    ...
    // Elementos <video> con src de Vimeo
  ...
  // Exponemos rescan para llamadas posteriores
