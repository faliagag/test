...
(async function () {
  ...
  async function triggerRescan() {
    ...
    // Inyectar inject.js en todos los frames
    try {
    ...
    // También ejecutar rescan si ya estaba inyectado
    try {
      await chrome.scripting.executeScript({
        target: { tabId: tab.id, allFrames: true },
  ...
  async function loadVideos(autoRescan = true) {
    ...
    try {
      // Pasar tabId explícitamente al background
      const data = await chrome.runtime.sendMessage({ action: 'GET_VIDEOS', tabId: tab.id });
      if (data?.videos?.length > 0) {
        renderVideos(data);
      } else if (autoRescan) {
        // Primera vez: intentar rescan automático
        await triggerRescan();
      } else {
        renderEmpty();
      }
